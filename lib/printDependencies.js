...
module.exports = function printDependencies(files, job) {
  ...
  function printViz(files, job) {
    ...
    function printClusters(tree, label, depth, override) {
      console.log('subgraph cluster_' + clusterId++, '{');
      console.log('  label = "' + (override || (label || '.')) + '/"');
      console.log('  style="filled";');
      console.log('  pad="2";');
      console.log('  penwidth="0.5";');
      console.log('  fillcolor="' + clusterFills[depth % clusterFills.length] + '"');
      Object.keys(tree.files).forEach(function (file) {
        console.log('  "' + path.relative(job.base, file) + '";');
      });

      Object.keys(tree.children).map(function (name) { printClusters(tree.children[name], label + ( label ? path.sep : '') + name, depth + 1); });
